...
(function($) {
    ...
    const __tameCurrencyFormat = function(options) {
        ...
        function __tameCheckCurrencySymbol(string){
            if ((typeof string === 'string' && $.trim(string).length > 0) || string === true) {
                return true;
            }
            return false;
        };
        ...
        function __tameGetStyleFormat(style){
            if ($.trim(style).toLowerCase() === 'currency') {
                return style;
            }else if($.trim(style).toLowerCase() === 'decimal'){
                return style;
            }
            return 'currency';
        };
        ...
        // get amount 
        var formattedNumber = intNumber.format(settings.number);

        // get an array of formatted parts of the number
        var matches = intNumber.formatToParts(settings.number);
